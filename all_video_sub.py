...
df = pd.read_excel('./id-download videos格式.xlsx')
...
for i in range(len(df)):
    folder = df.iloc[i][0]
    folder2 = folder.replace(' ','_')

    animal_name = df.iloc[i][1]
    animal_name2 = animal_name.replace(' ','_')
    action = df.iloc[i][2]
    action2 = action.replace(' ','_')
...
#     id_l_str = '$$'.join(id_list)
    ...
    file = Path('code')/f"run_{folder2}_{animal_name2}_{action2}.sh"
